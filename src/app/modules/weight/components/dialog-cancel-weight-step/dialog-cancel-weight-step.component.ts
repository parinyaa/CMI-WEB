...
export class DialogCancelWeightStepComponent implements OnInit {
  ...
  constructor(
    ...
    private cancelWeightStepRequest: CancelWeightStepRequest
  ) { }
  ...
  cancelWeightStep() {
    this.loading.show();
    this.cancelWeightStepRequest.stepId = this.cancelWeightStepForm.value.id
    this.cancelWeightStepRequest.cancelNote = this.cancelWeightStepForm.value.cancelNote
    this.weightService.cancelWeightStepById(this.cancelWeightStepRequest).subscribe(
      (res) => {
        this.loading.hide();
