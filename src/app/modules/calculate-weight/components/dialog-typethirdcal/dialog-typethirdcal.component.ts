...
export class DialogTypethirdcalComponent implements OnInit {
  ...
  onSubmitSelect() {
    ...
    this.dataResult.forEach(element => {
      sumPercent += parseFloat(element.weight);
    });
    if (sumPercent <= 0) {
      this.warningSwal.title = 'กรุณาเลือก';
      ...
      this.dataResult.forEach(element => {
        let percent: number = parseFloat(element.weight) / sumPercent;
      ...
      if (parseFloat(sumWeight.toFixed(5)) == weightParent) {
        ...
        let diffWeight: number = weightParent - sumWeight;
        ...
        // let weight: number = arrWeight + diffWeight;
